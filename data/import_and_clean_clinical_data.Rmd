...
# raw_clinical_data <- read.table("clinical_patient_laml.tsv", header = TRUE, sep = "\t")
```
...
```
This data-sheet has `r length(raw_clinical_data)` columns, a lot more variables than we'll need for this project. The `r nrow(raw_clincal_data)` rows represent the patients.
