...
rDat <- read.delim("../../data/aml.rnaseq.gaf2.0_read_count_cleaned.txt", row.names = 1, check.names = FALSE)
```

Preprocess data (normalize and log2) using voom:
```{r}
norm.factor <- calcNormFactors(rDat)
dat.voomed <- voom(rDat, design = NULL, lib.size = colSums(rDat) * norm.factor)
rDatNorm <- dat.voomed$E
```
...
fs.corr <- function(input.dat, input.labels) {
  gene.corrs <- apply(input.dat, 1, function(x) return(suppressWarnings(cor(x, as.numeric(input.labels)))))
...
rf.cv <- function(all.dat, all.labels, K = 5, fs.method = "lm", plot.varimp = TRUE) {
  ...
  conf_matrix <- matrix(0, nrow = 2, ncol = 2, dimnames = list(c("trueA", "trueB"), c("predA", "predB")))
  feature.list <- list()
