...
rf.cv <- function(all.dat, all.labels, all.levels, K = 5, fs.method = "lm", plot.varimp = TRUE, conf.mat.flip = FALSE) {
  set.seed(540)
...
rf.labels <- mapvalues(rfDes$Cytogenetic_risk, c("Good","Intermediate","Poor"), c(0,0,1), warn_missing = TRUE)
...
```{r}
cv.trisomy8.res <- rf.cv(rDat, factor(as.numeric(rDes$trisomy_8)), rf.levels, K = 5, fs.method = "lm")
cv.trisomy8.res[1:3]
...
```{r}
cv.del5.res <- rf.cv(rDat, factor(as.numeric(rDes$del_5)), rf.levels, K = 5, fs.method = "lm")
cv.del5.res[1:3]
...
```{r}
cv.del7.res <- rf.cv(rDat, factor(as.numeric(rDes$del_7)), rf.levels, K = 5, fs.method = "lm")
cv.del7.res[1:3]
...
First look at trisomy 8:
```{r}
cv.trisomy8.res <- rf.cv(rDat, factor(as.numeric(rDes$trisomy_8)), factor(as.numeric(rDes$trisomy_8)), K = 5, fs.method = "corr")
cv.trisomy8.res[1:3]
...
Next look at deletions of chromosome 5:
```{r}
cv.del5.res <- rf.cv(rDat, factor(as.numeric(rDes$del_5)), factor(as.numeric(rDes$del_5)), K = 5, fs.method = "corr")
cv.del5.res[1:3]
...
Next look at deletions of chromosome 7:
```{r}
cv.del7.res <- rf.cv(rDat, factor(as.numeric(rDes$del_7)), factor(as.numeric(rDes$del_7)), K = 5, fs.method = "corr")
cv.del7.res[1:3]
...
```{r}
rfDes <- rDes[rDes$Cytogenetic_risk != "N.D.",]
rf.labels <- mapvalues(rfDes$Cytogenetic_risk, c("Good","Intermediate","Poor"), c(1,0,0), warn_missing = TRUE)
rf.labels <- factor(rf.labels)
rf.levels <- mapvalues(rfDes$Cytogenetic_risk, c("Good","Intermediate","Poor"), c(3,2,1), warn_missing = TRUE)
rf.levels <- factor(rf.levels)
rfDat <- rDat[,rownames(rfDes)]
...
```{r}
rfDes <- rDes[rDes$Cytogenetic_risk != "N.D.",]
rf.labels <- mapvalues(rfDes$Cytogenetic_risk, c("Good","Intermediate","Poor"), c(0,1,0), warn_missing = TRUE)
rf.labels <- factor(rf.labels)
rf.levels <- mapvalues(rfDes$Cytogenetic_risk, c("Good","Intermediate","Poor"), c(3,2,1), warn_missing = TRUE)
rf.levels <- factor(rf.levels)
rfDat <- rDat[,rownames(rfDes)]
...
```{r}
cv.risk.res <- rf.cv(rfDat, rf.labels, rf.levels, K = 5, fs.method = "corr", conf.mat.flip = TRUE)
cv.risk.res[1:3]
