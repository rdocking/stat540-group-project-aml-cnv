...
```{r readCountAdd1Log2DensityPlot}
rDat <- rDat + 1
rDatMelt <- melt(rDat, variable.name = "Sample", 
                 value.name = "ReadCount")
...
```{r rDatCRGIPvoomMeanVarTrendPlot}
...
```{r CytoRisk_GoodvsPoor_TopSixBoxplot}
...
```{r CytoRisk_GoodvsInt_TopSixBoxplot}
...
```{r CytoRisk_IntvsPoor_TopSixBoxplot}
...
```{r CytoRiskDEA_allComparisons_FDR1e-5_VennDiagram}
...
```{r CytoRiskDEA_allComparisons_commonGene_Boxplot}
