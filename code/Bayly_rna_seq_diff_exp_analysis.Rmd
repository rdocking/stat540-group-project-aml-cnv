...
#will continue this...
nrow(corMelt)
corMelt <- subset(corMelt, RPKM < 1)
corMelt <- order(corMelt, decreasing = FALSE)
corMelt$rep <- c(1,2)
corMelt <- subset(corMelt, rep != 2)
#now we have a list of all correlation coefficients with names containing information. 
#It may have been better, for this, to only have sample names.
#then we could basically grep 1 sample (for a given cutoff) and get back all other samples with high correlation to that one.
```
...
Removing genes with max RPKM less than 8
```{r echo = FALSE}
del5Dat <- rDat[voomDel5genes,] #subsetting transcripts of interest from rDat
