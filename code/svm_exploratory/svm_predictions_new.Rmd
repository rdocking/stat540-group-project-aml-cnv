...
svm.labels <- mapvalues(svmDes$Cytogenetic_risk, c("Good","Intermediate","Poor"), c(0,0,1), warn_missing = TRUE)
...
Run a 5-fold cross-validation for the data:
```{r}
cv.risk.res <- svm.cv(svmDat, svm.labels, svm.levels, K = 5, fs.method = "lm", conf.mat.flip = TRUE)
cv.risk.res[1:3]
...
Run a 5-fold cross-validation for the data:
```{r}
cv.risk.res <- svm.cv(svmDat, svm.labels, svm.levels, K = 5, fs.method = "corr", conf.mat.flip = TRUE)
cv.risk.res[1:3]
