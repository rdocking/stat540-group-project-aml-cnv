...
First look at trisomy 8:
```{r}
cv.trisomy8.res <- svm.cv(rDat, factor(as.numeric(rDes$trisomy_8)), svm.levels, K = 5, fs.method = "corr")
cv.trisomy8.res[1:3]
fts <- cv.trisomy8.res[[4]]
...
Next look at deletions of chromosome 5:
```{r}
cv.del5.res <- svm.cv(rDat, factor(as.numeric(rDes$del_5)), svm.levels, K = 5, fs.method = "corr")
cv.del5.res[1:3]
fts <- cv.del5.res[[4]]
...
Next look at deletions of chromosome 7:
```{r}
cv.del7.res <- svm.cv(rDat, factor(as.numeric(rDes$del_7)), svm.levels, K = 5, fs.method = "corr")
cv.del7.res[1:3]
fts <- cv.del7.res[[4]]
