...
dat.in.read$prognosis <- mapvalues(dat.in.read$del_5, c("FALSE","TRUE"), c(0,1), warn_missing = TRUE)
dat.in.rpkm <- dat.filt.rpkm
dat.in.rpkm$prognosis <- mapvalues(dat.in.rpkm$del_5, c("FALSE","TRUE"), c(0,1), warn_missing = TRUE)
```
...
#     xtrain.supervised <- cbind(sup.tr, prelim.in[folds$subsets[folds$which!=f,],c("prognosis","del_5")])
#     xtest.supervised <- cbind(sup.ts, prelim.in[folds$subsets[folds$which==f,],c("prognosis","del_5")])
#     
...
princomp <- prcomp(prelim.in[,grepl(".*calculated",colnames(prelim.in))], center = T, scale = F)
smoothScatter(princomp$x) #generates a smooth scatter plot that shows the density of the data points.
plot(princomp$x[ ,c("PC1","PC2")], bg = as.numeric(prelim.in$del_5), pch = 21, cex = 1.5, main="Cytogenic_risk")
legend("topright", as.character(levels(factor(prelim.in$del_5))), pch = 21, pt.bg =c(1,2,4))
...
intersection_rpkm <- union(union(selected_gene_1, selected_gene_2), selected_gene_3)-intersect(intersect(selected_gene_1, selected_gene_2), selected_gene_3)
```
```{r}
...
scatter <- pc[,c("del_5", "Cytogenetic_risk", "del_7", "PC1", "PC2", "PC3")]
#scatter$del_5 <- as.numeric(scatter$del_5)
