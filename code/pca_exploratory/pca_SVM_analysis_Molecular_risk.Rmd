...
princomp <- prcomp(prelim.in[,grepl(".*calculated",colnames(prelim.in))], center = T, scale = F)
smoothScatter(princomp$x) #generates a smooth scatter plot that shows the density of the data points.
plot(princomp$x[ ,c("PC1","PC2")], bg = as.numeric(prelim.in$Molecular_risk), pch = 21, cex = 1.5, main="Cytogenic_risk")
legend("topright", as.character(levels(factor(prelim.in$Molecular_risk))), pch = 21, pt.bg =c(1,2,4))
...
stripplot(gExp ~ Molecular_risk | intersection[1:9] , prepareData(intersection[1:9], t(data), (des)), jitter.data = TRUE, auto.key = TRUE, type = c('p', 'a'), grid = TRUE, main= "Interesting hits")
