...
dat.in.read$prognosis <- mapvalues(dat.in.read$del_7, c("TRUE","FALSE"), c(1,0), warn_missing = TRUE)
dat.in.rpkm <- dat.filt.rpkm
dat.in.rpkm$prognosis <- mapvalues(dat.in.rpkm$del_7, c("TRUE","FALSE"), c(1,0), warn_missing = TRUE)
```
...
splom(scatter, http://127.0.0.1:35277/rstudio/clear.cache.gif
      pch=16, col=1,
      pscale=0,
      xlab=NULL,
...
plot(princomp$x[ ,c("PC1","PC2")], bg = as.numeric(prelim.in$Cytogenetic_risk), pch = 21, cex = 1.5, main="Cytogenetic_risk")
...
stripplot(gExp ~ del_7 | intersection[1:9] , prepareData(intersection[1:9], t(data), (des)), jitter.data = TRUE, auto.key = TRUE, type = c('p', 'a'), grid = TRUE, main= "Interesting hits")
