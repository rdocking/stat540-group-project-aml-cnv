...
```
**First, let's reorder the factor levels for Molecular_risk so they make sense.**
```{r}
levels(dat.complete.rpkm$Molecular_risk) <- c("Poor","Intermediate","Good","N.D.")
levels(dat.complete.read$Molecular_risk) <- c("Poor","Intermediate","Good","N.D.")

```
**Remove the rows with N.D.**
```{r}
dat.filt.rpkm <- subset(dat.complete.rpkm,dat.complete.rpkm$Molecular_risk != "N.D.")
dat.filt.read <- subset(dat.complete.read,dat.complete.read$Molecular_risk != "N.D.")
```

**let's make a binary prognosis columns (TRUE for poor).**
```{r}
dat.in.read <- dat.filt.read
dat.in.read$prognosis <- mapvalues(dat.in.read$Molecular_risk, c("Good","Intermediate","Poor"), c(0,0,1), warn_missing = TRUE)
dat.in.rpkm <- dat.filt.rpkm
dat.in.rpkm$prognosis <- mapvalues(dat.in.rpkm$Molecular_risk, c("Good","Intermediate","Poor"), c(0,0,1), warn_missing = TRUE)
```
...
prelim.in.read <- dat.in.read[,c(colnames(prelim.in.read),"prognosis","Molecular_risk")]

prelim.in.rpkm <- dat.in.rpkm[,grepl("_*calculated",colnames(dat.in.rpkm))]
gene.name.rpkm <- colnames(prelim.in.rpkm)
prelim.in.rpkm <- dat.in.rpkm[,c(colnames(prelim.in.rpkm),"prognosis","Molecular_risk")]
```
...
cross_validate <- function(prelim_in, d){ 
  ...
  for (f in 1:k) {
    ...
    xtrain <- t(res$tr.dat)
    xtest <- t(res$ts.dat)
    xtrain <- cbind(xtrain, prelim.in[folds$subsets[folds$which!=f,],c("prognosis","Molecular_risk")])
    xtest <- cbind(xtest, prelim.in[folds$subsets[folds$which==f,],c("prognosis","Molecular_risk")])
    ...
    xtrain.supervised <- cbind(sup.tr, prelim.in[folds$subsets[folds$which!=f,],c("prognosis","Molecular_risk")])
    xtest.supervised <- cbind(sup.ts, prelim.in[folds$subsets[folds$which==f,],c("prognosis","Molecular_risk")])
    ...
    xtrain.basic <- cbind(princomp$x[folds$subsets[folds$which!=f,], 1:d], prelim.in[folds$subsets[folds$which!=f,],c("prognosis","Molecular_risk")])
    xtest.basic <- cbind(princomp$x[folds$subsets[folds$which==f,], 1:d], prelim.in[folds$subsets[folds$which==f,],c("prognosis","Molecular_risk")])
