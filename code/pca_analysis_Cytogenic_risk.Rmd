...
cross_validate <- function(prelim_in, d){ 
  ...
  for (f in 1:k) {
    ...
    xtrain <- t(res$tr.dat)
    xtest <- t(res$ts.dat)
    ytr <- prelim.in[folds$subsets[folds$which!=f,],c("prognosis")]
    ytr <- as.numeric(levels(ytr))[ytr]
    xtrain <- cbind(xtrain, prognosis=ytr)
    yts <- prelim.in[folds$subsets[folds$which==f,],c("prognosis")]
    yts <- as.numeric(levels(yts))[yts]
    xtest <- cbind(xtest, prognosis=yts)
